...
struct client_t::impl_t
{
  struct state_t : public std::enable_shared_from_this<state_t>
  {
    ...
    virtual void send(msg::some_datagram_t data, bool prev_serial_needed) = 0;
    virtual void read() = 0;
  ...
  struct connect_state_t : public state_t
  {
    ...
    void send(msg::some_datagram_t data, const bool prev_serial_needed) override;
    void read() override;
  ...
  impl_t(const message_received_callback_t& callback, const server_status_changed_callback_t& server_status_changed, const prepare_msg_strategy_t& prepare_msg_strategy,
         std::unique_ptr<connection_strategy_t> connection_strategy);
  void resend_prev_message();
  void begin_wait_receive(const msg::some_datagram_t& data);
  ...
  void write_datagram(msg::some_datagram_t data, bool prev_serial_needed);

  const prepare_msg_strategy_t prepare_msg_fn_;
  udp_socket_t socket_;
  QTimer server_alive_timer_;

  QTimer response_checker_timer_;
  msg::some_datagram_t last_send_message_;
  std::vector<msg::some_datagram_t> messages_to_send_; // TODO: fix: not used!
...
client_t::client_t(const message_received_callback_t& callback, const client_t::server_status_changed_callback_t& server_status_changed,
                   const prepare_msg_strategy_t& prepare_msg_strategy, std::unique_ptr<connection_strategy_t> connection_strategy)
  : impl_(std::make_unique<impl_t>(callback, server_status_changed, prepare_msg_strategy, std::move(connection_strategy)))
{}

client_t::~client_t() = default;

void client_t::send(msg::some_datagram_t data)
{
  impl_->state_->send(std::move(data), false);
}
...
client_t::impl_t::impl_t(const client_t::message_received_callback_t& callback, const client_t::server_status_changed_callback_t& server_status_changed,
                         const prepare_msg_strategy_t& prepare_msg_strategy, std::unique_ptr<connection_strategy_t> connection_strategy)
  : prepare_msg_fn_{prepare_msg_strategy}
...
void client_t::impl_t::begin_wait_receive(const msg::some_datagram_t& data)
{
...
{
  client_.messages_to_send_.push_back(std::move(data));
}
...
void client_t::impl_t::connect_state_t::read()
{
  ...
  if (msg::id_v<msg::message_received_t> != datagram.data.type) return;
...
void client_t::impl_t::connected_state_t::send(msg::some_datagram_t data, const bool prev_serial_needed)
{
  if (msg::id_v<msg::message_received_t> != data.type)
  {
  ...
  client_.write_datagram(std::move(data), prev_serial_needed);
}
